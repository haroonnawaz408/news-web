...
export const AdminNewPost: React.FC = () => {
  ...
  const [tagsInput, setTagsInput] = useState('AI, Technology');
  const [excerpt, setExcerpt] = useState('');
  const [content, setContent] = useState(`## Executive Summary\n\nProvide an introductory overview of the breakthrough or event.\n\n### Architectural Specifications\n\n* **Primary Metric:** Benchmark score\n* **Latency Impact:** Real-time throughput\n\n\`\`\`python\n# Implementation snippet\ndef optimize_pipeline():\n    pass\n\`\`\`\n\n### Strategic Analysis\n\nDetail the broader consequences for the industry.`);
  const [featuredImage, setFeaturedImage] = useState('https://images.unsplash.com/photo-1518770660439-4636190af475?w=1200&auto=format&fit=crop&q=80');
  const [status, setStatus] = useState<'published' | 'draft'>('draft');
  const [scheduledAt, setScheduledAt] = useState('');
  const [metaTitle, setMetaTitle] = useState('');
  const [metaDescription, setMetaDescription] = useState('');
  const [authorName, setAuthorName] = useState('PulseNews Editorial Board');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const created = await createPost({
        ...
        category,
        tags,
        excerpt: excerpt || content.slice(0, 160).trim() + '...',
        content,
      ...
      toast(`Article "${created.title}" successfully created!`, 'success');
      navigate('/admin/posts');
    } catch {
      toast('Failed to create article. Please check your fields.', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-[#070B0E] py-8">
      <SEO noindex title="Create New Article" description="Draft a new investigative news report on PulseNews Pakistan." />
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <form onSubmit={handleSave} className="space-y-6">
          <div className="bg-white dark:bg-[#111827] p-6 rounded-xl border border-neutral-200 dark:border-neutral-800 space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="Next-Generation Frontier Models: How Autonomous Reasoning Transforms Engineering"
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="A concise, high-impact summary displayed across news feeds and social embeds..."
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="AI, Reasoning, Hardware, Benchmark"
                    className="w-full px-3 py-2 rounded-lg border border-neutral-300 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800 text-neutral-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
          ...
          {/* Markdown Content & Live Preview Tabs */}
